...
from blink import Blink, AsyncBlink, DefaultAioHttpClient
from blink._utils import is_dict
...
logging.getLogger("blink").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Blink]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Blink(base_url=base_url, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncBlink]:
    ...
    async with AsyncBlink(base_url=base_url, _strict_response_validation=strict, http_client=http_client) as client:
        yield client
